...
with tab_bilanse:
    ...
    bilans_typ = st.radio(
        "Wybierz raport:",
        ["🚛 Kurierzy: faktury vs przelewy (12mc)", "📦 Koszt części: top 15 skrzyń (10mc)", "💸 Rentowność: najgorsza marża"],
        horizontal=True,
        key="bilans_typ"
    )
    ...
    if "🚛" in bilans_typ:
        ...
        if st.button("🔄 Pobierz świeże dane", key="kurierzy_pobierz"):
            st.session_state.pop("_bilans_kurierzy_dane", None)
        
        if "_bilans_kurierzy_dane" not in st.session_state:
        ...
        if dane is None:
            ...
            for k in kurierzy:
                ...
                fig.update_layout(
                    title=dict(text=f"<b>{k}</b> — porównanie kwot zafakturowanych z realnie wypłaconymi (tys. PLN)", font=dict(color='#2C3E50')),
                    barmode='group',
                    height=350,
                    legend=dict(orientation='h', y=1.15, x=0.5, xanchor='center', font=dict(color='#2C3E50')),
                    plot_bgcolor='white',
                    paper_bgcolor='white',
                    font=dict(color='#2C3E50', size=12),
                    yaxis=dict(title=dict(text='tys. PLN', font=dict(color='#2C3E50')), gridcolor='#ECF0F1', tickfont=dict(color='#2C3E50')),
        ...
        if st.button("🔄 Pobierz świeże dane", key="skrzynie_pobierz"):
            st.session_state.pop("_bilans_skrzynie_dane", None)
        
        if "_bilans_skrzynie_dane" not in st.session_state:
        ...
        if df_skrz is None or df_skrz.empty:
            ...
            fig.update_layout(
                height=500,
                legend=dict(orientation='v', y=1, x=1.02, font=dict(color='#2C3E50')),
                plot_bgcolor='white',
                paper_bgcolor='white',
                font=dict(color='#2C3E50', size=12),
                yaxis=dict(title=dict(text='średni koszt zł/szt', font=dict(color='#2C3E50')), gridcolor='#ECF0F1', tickfont=dict(color='#2C3E50')),
                xaxis=dict(gridcolor='#ECF0F1', tickfont=dict(color='#2C3E50')),
            ...
            fig_heat.update_layout(
                ...
                paper_bgcolor='white',
                font=dict(color='#2C3E50', size=12),
                yaxis=dict(autorange='reversed', tickfont=dict(color='#2C3E50')),
    ...
    else:  # 💸 Rentowność najgorsza marża
        ...
        if st.button("🔄 Pobierz świeże dane", key="rentownosc_pobierz"):
            st.session_state.pop("_bilans_rentownosc_dane", None)
        
        if "_bilans_rentownosc_dane" not in st.session_state:
        ...
        if dane is None:
            ...
            if df_sprz.empty or df_kszt.empty:
                ...
                fig.update_layout(
                    ...
                    font=dict(color='#2C3E50', size=12),
                    yaxis=dict(
                        title=dict(text='Marża /szt (PLN)', font=dict(color='#2C3E50')),
                        gridcolor='#ECF0F1', 
