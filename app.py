...
==========================================================================
🔵 MS SQL SERVER — DANE SPRZEDAŻOWE (dialekt T-SQL!)
==========================================================================

Firma ma 3 bazy MSSQL. Każda ma inne przeznaczenie — wybierz właściwą:

**📦 ebayApiDB** — sprzedaż na eBay:
- zamówienia z eBay, transakcje, prowizje
- pytania typu: "ile zamówień na eBay w ostatnim tygodniu", "średnia cena sprzedaży eBay", "prowizje eBay"
- pewnie tabele typu: [dbo].[Orders], [dbo].[Transactions], [dbo].[Fees]

**💰 STEEPC** — ceny katalogowe, skupy, wyceny:
- ceny podstawowe sprzedażowe wszystkich kanałów
- skupy używanych skrzyń (austausch → PRZYJĘCIE od klienta)
- prowizje i zarobki
- pytania typu: "ile skrzyń skupiliśmy", "jaka cena katalogowa X", "prowizje za marzec"

**🛒 SHOP_PMG** — sklep własny shopPL/shopEU:
- zamówienia z własnego sklepu (autosparts-kupisz, podobnie)
- pytania typu: "sprzedaż shopPL vs shopEU", "zamówienia z własnego sklepu"
...
⚠️ DIALEKT T-SQL — NIE UŻYWAJ SKŁADNI POSTGRESA:
  ...
  ❌ STRING_AGG(x, ',')         → ✅ STRING_AGG(x, ',') (działa w MSSQL 2017+)
...
SELECT COUNT(*) FROM [SHOP_PMG].[dbo].[Zamowienia] WHERE [Status] = 'Zrealizowane'

⚠️ GDY PYTANIE DOTYCZY OBU BAZ:
Nie da się zrobić jednego zapytania SQL łączącego Postgres i MSSQL bezpośrednio.
Jeśli użytkownik pyta np. o "zysk per skrzynia" (potrzebujesz i kosztu z PG i ceny z MSSQL):
- Albo wybierz stronę bliższą pytaniu
- Albo zwróć zapytanie dla jednej strony (preferuj tę która ma więcej wymaganych informacji)
...
def mssql_connect(tunnel, database):
    """Połączenie z konkretną bazą SQL Server przez tunel.
    Wymusza wersję TDS 7.4 (najbardziej kompatybilna z nowszym SQL Server)."""
    import pymssql
    return pymssql.connect(
        ...
        login_timeout=15,
        charset='UTF-8',
        tds_version='7.4',  # Wymusza protokół TDS 7.4 (SQL Server 2012+)
    )
...
def diagnostyka_mssql():
    ...
    if not zaladowana_wersja:
        ...
        kroki.append({"krok": "💡 Wniosek", "status": "❌",
                      "info": "Żadna wersja TDS nie łapie. Problem głębiej — może dane logowania, może serwer wymaga Windows Auth, może to nie SQL Server."})
        return kroki
...
def pobierz_schema_mssql():
    ...
    Zwraca dict z kluczami 'bazaMSSQL.schema.table' (zachowujemy pełny klucz żeby odróżnić od Postgresa)."""
    try:
        import pymssql
    except ImportError:
        return None
            ...
            for db_name in databases:
                try:
                    ...
                    for tschema, tname, ttype in tables:
                        ...
                        """, (tschema, tname))
                        cols = cur.fetchall()
                        # Klucz: [MSSQL] db.schema.table żeby odróżnić od Postgresa
                    ...
                    st.warning(f"SQL Server baza {db_name}: {str(e)[:150]}")
                    continue
    except Exception as e:
...
def wykryj_silnik_z_sql(sql):
    ...
    # 1) Jawne sygnały MSSQL w zapytaniu
    mssql_databases = [d.lower() for d in st.secrets.get("MSSQL_DATABASES", ["ebayApiDB", "STEEPC", "SHOP_PMG"])]
    for db_lower in mssql_databases:
        # Szukamy wzorca [ebayApiDB].[dbo].[tabela] lub ebayApiDB.dbo.tabela
        if re.search(rf'\b{re.escape(db_lower)}\b', sql_lower):
            original_db = next((d for d in st.secrets.get("MSSQL_DATABASES", []) if d.lower() == db_lower), db_lower)
            return ("mssql", original_db)

    # 2) Specyficzne słowa MSSQL (T-SQL)
    if re.search(r'\btop\s+\d+\b', sql_lower) and 'limit' not in sql_lower:
        return ("mssql", "ebayApiDB")  # TOP bez LIMIT = T-SQL
    if 'getdate()' in sql_lower or 'isnull(' in sql_lower:
        return ("mssql", "ebayApiDB")

    # 3) Specyficzne dla Postgresa
    if '::' in sql or 'current_date' in sql_lower or 'interval' in sql_lower:
        return "postgres"

    # 4) Domyślnie Postgres (bo jest naszą głównę bazą)
    return "postgres"
