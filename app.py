...
Streamlit Cloud → SSH tunel → PostgreSQL → Firestore (chunki) → wykresy
"""
    ...
    try:
        with otworz_tunel() as tunnel:
            ...
            if full_query:
                ...
                progress.progress(40, text="🔍 Szukam widoków...")
                candidates = []
...
def zapisz_do_firestore(df, meta):
    ...
    # Wyczyść stare chunki dla latest
    try:
    ...
    try_write(db.collection("rentownosc_raporty").document("latest"), {"meta": meta}, "meta latest")
    ...
    for i in range(num_chunks):
        ...
        progress.progress((i + 1) / num_chunks, text=f"💾 Chunk {i+1}/{num_chunks}")

    # Snapshot historyczny (best effort)
    try:
        try_write(db.collection("rentownosc_raporty").document(snap_id), {"meta": meta}, "meta snap")
        for i in range(num_chunks):
            chunk = records[i * CHUNK_SIZE : (i + 1) * CHUNK_SIZE]
            ref = db.collection("rentownosc_raporty").document(snap_id).collection("chunks").document(f"chunk_{i:04d}")
            try_write(ref, {"data": chunk, "index": i, "size": len(chunk)}, f"snap chunk {i}")
    except Exception as e:
        st.caption(f"Snapshot pominięty: {e}")

    progress.empty()

    # Rotacja snapshotów (max 10)
    try:
...
def pobierz_z_firestore(doc_id="latest"):
    ...
    records = payload.get("data", [])  # stary format
        ...
        with st.spinner("Łączenie z produkcją..."):
            ...
            if df_new is not None:
                try:
                    ...
                    st.cache_data.clear()
                    st.success(f"✅ Zapisano {meta_new['row_count']} wierszy w {meta_new.get('num_chunks', 1)} częściach")
                    st.rerun()
                except Exception as e:
                    st.error(f"❌ Zapis do Firestore: {e}")
                    st.info(f"📊 Dane pobrane ({len(df_new)} wierszy) — preview poniżej:")
...
# --- Jeśli nie ma grupaIgo — generyczny widok ---
if not COL_GRUPA:
    st.info("ℹ️ Ten widok nie ma kolumny 'grupaIgo' — pokazuję dane surowe z szybką agregacją.")
    st.subheader(f"📋 {meta.get('source_view', 'Dane')}")
    st.caption(f"Wierszy: {len(df)} | Kolumny: {len(df.columns)}")
    ...
    if numeric_cols and non_numeric:
        ...
        with col1:
            group_col = st.selectbox("Grupuj po:", non_numeric)
        with col2:
            agg_col = st.selectbox("Agreguj:", numeric_cols)

        if group_col and agg_col:
            try:
                agg = df.groupby(group_col)[agg_col].agg(['sum', 'mean', 'count']).reset_index()
                agg = agg.sort_values('sum', ascending=False).head(top_n)
                fig = go.Figure()
                fig.add_trace(go.Bar(x=agg[group_col].astype(str), y=agg['sum'],
                                    name=f"Suma {agg_col}", marker_color="#3B82F6"))
                fig.update_layout(title=f"{agg_col} per {group_col} (Top {top_n})",
                                 height=500, template="plotly_white",
                                 xaxis_tickangle=-45, margin=dict(b=120))
                st.plotly_chart(fig, use_container_width=True)
                st.dataframe(agg, use_container_width=True)
            except Exception as e:
                st.error(f"Agregacja: {e}")

    st.stop()

# --- Pełny dashboard rentowności ---
if COL_ILE:
...
if show_zysk:
    st.subheader("💰 Zysk na skrzyni per kanał")
    fig = go.Figure()
...
if show_cena_vs_koszt and COL_CENA_EBAY and COL_KOSZT_EBAY:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(name="Cena sprzedaży eBay", x=df_top[COL_GRUPA], y=df_top[COL_CENA_EBAY],
...
if show_porownanie_cen:
    st.subheader("📈 Średnia cena sprzedaży")
    cL, cR = st.columns(2)
    with cL:
        f = go.Figure()
