...
with tab_bilanse:
    ...
    bilans_typ = st.radio(
        "Wybierz raport:",
        ["🚛 Kurierzy: faktury vs przelewy (12mc)", "📦 Koszt części: top 15 skrzyń (10mc)", "💸 Rentowność: najgorsza marża", "🔔 Audyt dzwonka rentowności"],
        horizontal=True,
        key="bilans_typ"
    )
    ...
    if "🚛" in bilans_typ:
    ...
    else:  # 🔔 Audyt dzwonka rentowności
        ...
        if st.button("🔄 Pobierz świeże dane", key="dzwonek_pobierz"):
            st.session_state.pop("_bilans_dzwonek_dane", None)
        
        if "_bilans_dzwonek_dane" not in st.session_state:
