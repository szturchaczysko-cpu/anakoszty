...
- "skrzynia", "numer partii", "nr partii" → kolumna "proNrPartii" (w wielu tabelach)
- "index skrzyni", "indeks skrzyni", "indeks artykułu" → kolumna "artIndeks" w "public"."wmsArtykuly" (przykłady: "105FSI5GRUP2", "207DQ500GRUP5_BM", "156TL4GRUP12_USZK" — to są indeksy magazynowe skrzyń i części)
- "typ skrzyni", "grupa skrzyń", "model skrzyni" → kolumna "grupaIgo" (grupa igorowa)
- "nazwa skrzyni" → kolumna "artNazwa" w "public"."wmsArtykuly" (np. "Skrzynia biegów 1.4", "Skrzynia biegów 2.0 TDI")
- UWAGA: "zadprElementNrPartii" w "wmsZleceniaZadaniaProdukcyjne" to NIE jest index skrzyni — to kod znakowania nabijany na skrzynię przez twórcę (typu "1XU7", "26G1") — nie używaj tego jako index skrzyni!

### TWÓRCA (PRACOWNIK):
- "twórca", "twórca skrzyniowy", "pracownik produkcji", "montażysta" → kolumna "TworcaMaggo" lub join przez "public"."wmsUzytkownicy" (identyfikator użytkownika)
- Tabele z przypisaniem twórców do zleceń: "wmsZleceniaZadaniaProdukcyjnePracownicy", "wmsZleceniaZadaniaProdukcyjnePracownicyHist"

### CZĘŚCI POBRANE Z MAGAZYNU (bardzo ważne — częste pytanie!):
...
⚠️ PRAWIDŁOWE POWIĄZANIA (ZWERYFIKOWANE EMPIRYCZNIE):

1. **NUMER PARTII (INDEX SKRZYNI)**: preferuj "pak.zadprNrPartii" w "wmsPakunkiHist" — to jest NUMER PARTII SKRZYNI bezpośrednio, bez żadnych JOIN-ów!
   Przykład: SELECT pak."zadprNrPartii" AS "Index skrzyni" FROM "public"."wmsPakunkiHist" AS pak
   
2. **TWÓRCA SKRZYNIOWY**: kolumna "pakUserTw" w "wmsPakunkiHist" (login, np. "dawid_p", "damian_z")

3. **DATA POBRANIA**: "pakDataWyd" (timestamp) — używaj ::date żeby porównać z datą

4. **NAZWA CZĘŚCI**: JOIN do "wmsArtykuly" przez "artID", kolumna "artNazwa"

5. **ILOŚĆ**: "pakpIlosc" w "wmsPakunkiPozycjeHist"

⚠️ PUŁAPKI — BARDZO WAŻNE:
- W "wmsPakunkiHist" JEST kolumna "zlecID" (wielkie ID)
- W "wmsZleceniaProdukcyjneHist" i "wmsZleceniaProdukcyjne" NIE MA kolumny "zlecID" — jest tylko "zlecprID"!
- DLATEGO: jeśli chcesz łączyć pakunek ze zleceniem produkcyjnym, użyj pak."zlecprID" = zlec."zlecprID" (obie tabele mają zlecprID), ALE uwaga że pak."zlecprID" często jest puste!
- NAJPROŚCIEJ: nie rób JOIN do zleceń, weź numer partii bezpośrednio z pak."zadprNrPartii"

### 📋 SZABLON WZORCOWY dla pytań "co pobrano dziś w podziale na twórcę i skrzynię/index":

SELECT
    pak."pakUserTw" AS "Twórca",
    art."artIndeks" AS "Index skrzyni",
    art."artNazwa" AS "Nazwa części",
    SUM(poz."pakpIlosc") AS "Ilość"
...
WHERE pak."pakDataWyd"::date = '2026-04-23'
GROUP BY pak."pakUserTw", art."artIndeks", art."artNazwa"
ORDER BY pak."pakUserTw", art."artIndeks"
LIMIT 1000

KLUCZOWE: "index skrzyni" to "artIndeks" w "wmsArtykuly" — NIE jest to "zadprNrPartii" ani "proNrPartii" pakunku!
...
def schema_do_tekstu_dla_ai(schema, max_tables=100):
    if not schema:
        return ""
    lines = []
    ...
    for key in sorted_keys:
        ...
        # Kluczowe tabele — pełna lista kolumn; inne — do 30
        ...
        lines.append(f'{type_label} "{key}"{marker}: {cols_str}')
    return "\n".join(lines)
...
⚠️ KRYTYCZNA ZASADA — CUDZYSŁOWY (PostgreSQL case-sensitive):
W tej bazie WSZYSTKIE nazwy tabel i kolumn używają wielkich liter w środku (camelCase/PascalCase). PostgreSQL domyślnie zamienia wszystko na lowercase, więc MUSISZ używać cudzysłowów WSZĘDZIE gdzie nazwa zawiera wielką literę.
...
POPRAWNE przykłady:
  ...
  JOIN "public"."wmsPakunkiHist" AS T2 ON T2."pakZlecId" = T1."zlecId"

BŁĘDNE przykłady (nigdy tak NIE rób):
  T1.proNrPartii         ❌ (brak cudzysłowów → postgres czyta jako pronrpartii → błąd)
  FROM public.wmsPakunki ❌ (brak cudzysłowów na schema/tabeli)
  SELECT NazwaKolumny    ❌ (brak cudzysłowów na kolumnie)

Cudzysłowy obejmują TYLKO nazwę identyfikatora (tabeli lub kolumny), nie cały wyraz z kropką.
Alias tabeli (T1, T2, a, b) NIE wymaga cudzysłowów — tylko nazwa kolumny po kropce: T1."KolumnaWielkieLitery"

ZASADY OGÓLNE:
1. Zwracaj TYLKO zapytanie SELECT — nigdy INSERT/UPDATE/DELETE/DROP/TRUNCATE/ALTER/CREATE.
2. Zawsze dodawaj LIMIT 1000 na końcu (chyba że użytkownik prosi o konkretny limit lub pełną agregację).
3. Używaj jawnych nazw schematów z cudzysłowami: "public"."wmsArtykuly", "rapdb"."VwCos".
4. Preferuj gotowe widoki z rapdb gdy to możliwe — ale tylko jeśli pasują semantycznie (patrz słownik).
5. Dla dat używaj formatu DATE '2026-04-23' lub 'YYYY-MM-DD'::date.
6. Aliasy w SELECT też w cudzysłowach jeśli mają wielkie litery: AS "Nazwa Aliasu".
7. Wynik to JEDNO zapytanie SQL, bez komentarzy, bez markdown, bez średnika na końcu.
...
with tab_ai:
    ...
    with st.expander(f"📚 Schemat ({len(schema)} tabel/widoków)", expanded=False):
