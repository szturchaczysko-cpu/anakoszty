...
⚠️ UPRAWNIENIA: Użytkownik artur_ro ma dostęp TYLKO do 3 baz MSSQL:
1. **ebayApiDB** — sprzedaż eBay (134 tabele)
2. **STEEPC** — główna baza biznesowa, skupy, austausche, ceny (291 tabel) ⭐ GŁÓWNA
3. **SHOP_PMG** — sklep własny shopPL/shopEU (88 tabel)

🚫 NIE PYTAJ o bazy: AUSTAUCH-ROZLICZENIE, WYPLATOMAT, EdytorCen, KADRY itp.
(są widoczne w liście sys.databases ale artur_ro dostaje 18456 "Login failed" przy próbie dostępu)

**Gdzie szukać czego:**
- "sprzedaż eBay", "zamówienia eBay", "prowizje eBay" → **ebayApiDB**
- "sprzedaż własny sklep", "shopPL", "shopEU" → **SHOP_PMG**
- "austausche", "skupy", "ceny katalogowe", "wyceny ekspertyz", 
  "zamówienia klientów", "allegro" → **STEEPC** (to główna baza biznesowa — TU SZUKAJ)

**Baza STEEPC jest OLBRZYMIA (291 tabel)** — zanim generujesz SQL dla pytania "pokaż austausche", 
najpierw zapytaj sam "pokaż tabele w STEEPC zawierające słowo 'austausch' albo 'skup'":
SELECT TABLE_SCHEMA, TABLE_NAME FROM [STEEPC].INFORMATION_SCHEMA.TABLES 
WHERE TABLE_NAME LIKE '%austausch%' OR TABLE_NAME LIKE '%skup%' OR TABLE_NAME LIKE '%zwrot%'
...
-- Lista tabel w bazie STEEPC zawierających słowo 'skup':
SELECT TOP 100 TABLE_SCHEMA, TABLE_NAME FROM [STEEPC].INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE '%skup%'

-- Zamówienia eBay z ostatnich 7 dni:
SELECT TOP 1000 * FROM [ebayApiDB].[dbo].[Orders] WHERE [OrderDate] >= DATEADD(day, -7, GETDATE())

-- Sprzedaż w sklepie własnym za kwiecień:
SELECT TOP 1000 * FROM [SHOP_PMG].[dbo].[Zamowienia] WHERE [DataZamowienia] >= CAST('2026-04-01' AS DATE)

⚠️ NAZWY BAZ Z MYŚLNIKAMI: nigdy nie używaj — artur_ro nie ma uprawnień.
Jeśli pytanie wymaga danych z bazy z myślnikiem (AUSTAUCH-ROZLICZENIE, EKSPERTYZY-HELPER, GENERATOR-TYTULOW) —
ZAWSZE pytaj o odpowiednik w STEEPC lub innej dostępnej bazie.
...
Pytanie: "pokaż tabele w STEEPC dotyczące cen"
SQL: SELECT TOP 100 TABLE_SCHEMA, TABLE_NAME FROM [STEEPC].INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE '%cen%' OR TABLE_NAME LIKE '%price%'
"""
