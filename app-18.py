...
- wmsPakunkiPozycjeHist = konkretne pozycje (artykuły) w pakunku, łączymy przez "pakId"
...
def popraw_sql_po_bledzie(pierwotny_sql, blad, pytanie_pl, schema_text):
    ...
    prompt = f"""Jesteś ekspertem SQL dla PostgreSQL. Poprzednie zapytanie padło z błędem — popraw je.
...
⚠️ NAJCZĘSTSZY BŁĄD — BRAK CUDZYSŁOWÓW:
PostgreSQL jest case-sensitive i domyślnie zamienia nazwy na lowercase. Jeśli błąd mówi "kolumna xyz nie istnieje" a oryginalnie było XyzAbc — znaczy że brakło cudzysłowów.

Przykład NAPRAWY:
  BŁĄD:  T1.proNrPartii  →  POPRAWKA:  T1."proNrPartii"
  BŁĄD:  FROM public.wmsPakunki  →  POPRAWKA:  FROM "public"."wmsPakunki"
  BŁĄD:  SELECT TworcaMaggo  →  POPRAWKA:  SELECT "TworcaMaggo"

ZASADY:
1. Zwróć TYLKO poprawione zapytanie SELECT, nic więcej.
2. Zadbaj o cudzysłowy przy KAŻDYM identyfikatorze z wielką literą.
3. Jeśli błąd dotyczy nieistniejącej tabeli/kolumny — znajdź podobną w schemacie i użyj.
4. Zachowaj intencję oryginalnego pytania użytkownika.
5. Dodaj LIMIT 1000 jeśli brakuje.
6. Bez markdown, bez komentarzy, bez średnika."""
