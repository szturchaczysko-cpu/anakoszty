...
- Powiązanie ze zleceniem produkcyjnym (czyli skrzynią) idzie przez "pakZlecId" lub "zlecId"
...
def schema_do_tekstu_dla_ai(schema, max_tables=100):
    ...
    for key in sorted_keys:
        ...
        cols_str = ", ".join([f'{c["name"]} ({c["type"]})' for c in info["columns"][:30]])
        if len(info["columns"]) > 30:
            cols_str += f" ... +{len(info['columns'])-30} więcej"
...
def popraw_sql_po_bledzie(pierwotny_sql, blad, pytanie_pl, schema_text):
    """Prosi AI o poprawę SQL po błędzie z bazy."""
    try:
        from vertexai.generative_models import GenerativeModel
    except ImportError:
        return None, "Brak biblioteki google-cloud-aiplatform"
...
2. **WSKAZÓWKA HINT z PostgreSQL**: Jeśli błąd zawiera "HINT: Być może chodziło ci o X" — użyj DOKŁADNIE tego X. PostgreSQL sam podpowiada poprawną nazwę!
   Przykład:
     BŁĄD: kolumna poz.pakId nie istnieje. HINT: Być może chodziło ci o "poz.pakID"
     POPRAWKA: zmień "pakId" na "pakID" i zostaw resztę bez zmian

3. **CZĘSTE PUŁAPKI NAZEWNICTWA w tej bazie**:
   - Identyfikatory kończące się na "ID" są PISANE WIELKIMI LITERAMI: "pakID", "artID", "zlecID" (NIE pakId/artId/zlecId)
...
4. **CUDZYSŁOWY OBOWIĄZKOWE**:
   ...
   - Aliasy w SELECT AS "Moja Nazwa" — w cudzysłowach jeśli mają spację/wielką literę

5. **NIE ZMIENIAJ STRATEGII** chyba że błąd dotyczy całej tabeli (nie istnieje). Jeśli błąd dotyczy tylko kolumny — popraw tylko tę kolumnę, resztę zostaw!
...
def odpal_z_self_healing(sql, pytanie_pl, schema_text, max_retries=3):
    ...
    if "ai_sql" in st.session_state:
        ...
        if c1.button("▶️ Odpal zapytanie", type="primary"):
            if not can_refresh:
                ...
                df_ai, err, attempts = odpal_z_self_healing(edited_sql, pytanie_orig, schema_text, max_retries=3)
