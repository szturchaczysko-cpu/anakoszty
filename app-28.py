...
==========================================================================
🔵 MS SQL SERVER — DANE SPRZEDAŻOWE I BIZNESOWE (dialekt T-SQL!)
==========================================================================

Firma ma 20 baz MSSQL (serwer 192.168.3.138). Każda ma osobne przeznaczenie.
Wybór bazy ma KRYTYCZNE znaczenie — pytanie o sprzedaż idzie do innej bazy niż pytanie o austausche.

**📦 ebayApiDB** — 134 tabel, dane sprzedażowe z eBay:
- zamówienia eBay, transakcje, prowizje API
- pytania: "ile zamówień na eBay", "średnia cena eBay", "prowizje eBay"

**💰 STEEPC** — 291 tabel (GIGANT!), główna baza biznesowa:
- ceny katalogowe, skupy używanych skrzyń (!!), wyceny eksperckie
- pytania: "ile skrzyń skupiliśmy", "cena katalogowa X", "wyceny ekspertyz"
- tutaj pewnie siedzi słownik artykułów i cen dla wszystkich kanałów

**🛒 SHOP_PMG** — 88 tabel, sklep własny:
- zamówienia shopPL / shopEU (autosparts-kupisz)
- pytania: "sprzedaż z własnego sklepu", "zamówienia shop"

**🎯 AUSTAUCH-ROZLICZENIE** — baza austauschów (ZWROTY/WYMIANY!):
- rozliczenia austauschów od klientów (zamyka transakcje sprzedażowe)
- pytania: "ile austauschów przyjęliśmy", "austausch dla zlecenia X"
- TU PEWNIE JEST ODPOWIEDŹ NA "austausch vs skup" — patrz tabele w tej bazie

**💵 WYPLATOMAT** — wypłaty i rozliczenia pracownicze:
- wypłaty twórców, kalkulacje wynagrodzeń, punkty produkcyjne → pieniądze
- pytania: "ile zarobił twórca X w marcu", "wypłaty za tygodień"

**💲 EdytorCen** — edycja cen:
- narzędzie do zarządzania cenami, historia zmian
- pytania: "zmiany cen ostatnio", "kto edytował cenę X"

**📊 RAPDB** — raporty biznesowe (analogia do Postgresowego rapdb):
- pewnie gotowe widoki raportowe (np. dashboard sprzedaży, KPI)

**🔬 EKSPERTYZY-HELPER** — narzędzie ekspertyz:
- pytania: "ekspertyzy z ostatniego tygodnia", "typ ekspertyzy"

**📑 GENERATOR-TYTULOW** — generator tytułów aukcji
**🏷️ STEEPW** — (pewnie kopia/wersja STEEPC)
**📅 CALENDAR** — kalendarz firmowy
**👥 KADRY** — pracownicy
**📸 GALERIE360 / GalerieCV** — zdjęcia produktów
**🖥️ PULPIT** — pulpit użytkownika
**💼 FORFOR, JOBAGENT, RFID, WWMSDB** — specjalistyczne
**🤖 SZTURCHACZ** — druga aplikacja AI (customer service)

⚠️ JAK ROZPOZNAĆ KTÓREJ BAZY UŻYĆ:
- "austausch", "zwrot używanej skrzyni" → **AUSTAUCH-ROZLICZENIE**
- "zarobki twórcy", "wypłata", "ile zarobił" → **WYPLATOMAT**
- "cena X", "wycena", "zmiana cennika" → **EdytorCen** lub **STEEPC**
- "sprzedaż eBay" → **ebayApiDB**
- "sprzedaż shopPL/shopEU/własny sklep" → **SHOP_PMG**
- "skup skrzyń", "katalog artykułów MSSQL" → **STEEPC**
- "raport sprzedażowy" → **RAPDB**
...
POPRAWNE PRZYKŁADY MSSQL:
-- Pierwsze rozpoznanie bazy:
SELECT TOP 100 TABLE_SCHEMA, TABLE_NAME FROM [AUSTAUCH-ROZLICZENIE].INFORMATION_SCHEMA.TABLES ORDER BY TABLE_NAME
-- Zamówienia eBay:
SELECT TOP 1000 [OrderId], [Total] FROM [ebayApiDB].[dbo].[Orders] WHERE [OrderDate] >= DATEADD(day, -7, GETDATE())
-- Skupy z STEEPC:
SELECT TOP 50 * FROM [STEEPC].[dbo].[Skupy] WHERE [DataSkupu] >= CAST('2026-04-01' AS DATE)
-- Austausche per dzień:
SELECT CAST([DataPrzyjecia] AS DATE) AS "Dzień", COUNT(*) AS "Ilosc" FROM [AUSTAUCH-ROZLICZENIE].[dbo].[Austausche] WHERE [DataPrzyjecia] >= DATEADD(day, -30, GETDATE()) GROUP BY CAST([DataPrzyjecia] AS DATE) ORDER BY "Dzień" DESC

⚠️ GDY PYTANIE DOTYCZY OBU BAZ (Postgres+MSSQL):
Nie da się zrobić jednego zapytania łączącego oba silniki. Jeśli użytkownik pyta np. o "zysk per skrzynia":
- Albo wybierz stronę bliższą pytaniu
- Albo zwróć zapytanie dla jednej strony (zwykle strony z "wartościami" czyli MSSQL)

Pytanie: "ile zamówień na ebay wpłynęło dzisiaj"
SQL: SELECT COUNT(*) AS "Ilosc" FROM [ebayApiDB].[dbo].[Orders] WHERE CAST([OrderDate] AS DATE) = CAST(GETDATE() AS DATE)

Pytanie: "pokaż listę tabel w bazie AUSTAUCH-ROZLICZENIE"
SQL: SELECT TABLE_SCHEMA, TABLE_NAME, TABLE_TYPE FROM [AUSTAUCH-ROZLICZENIE].INFORMATION_SCHEMA.TABLES ORDER BY TABLE_NAME

Pytanie: "pokaż 20 ostatnich skupów używanych skrzyń"
SQL: SELECT TOP 20 * FROM [STEEPC].[dbo].[Skupy] ORDER BY [DataSkupu] DESC
"""
...
def wykryj_silnik_z_sql(sql):
    ...
    # 1) Jawne odwołanie do bazy MSSQL w zapytaniu (np. [AUSTAUCH-ROZLICZENIE].[dbo].[...])
    for db in mssql_databases:
        # Szukamy pełnego nazewnictwa [nazwa] lub bez nawiasów
        pattern = rf'\[?{re.escape(db)}\]?\s*\.\s*(\[?\w+\]?\s*\.)?\s*\[?\w'
        if re.search(pattern, sql, re.IGNORECASE):
            return ("mssql", db)
        # Spójrz też na sam identyfikator nazwy bazy bez dot
        if re.search(rf'\b{re.escape(db)}\b', sql, re.IGNORECASE):
            return ("mssql", db)

    # 2) Specyficzne słowa MSSQL (T-SQL)
    mssql_sygnaly = [
        r'\btop\s+\d+\b',      # TOP N (bez LIMIT)
        r'\bgetdate\s*\(\s*\)',  # GETDATE()
        r'\bisnull\s*\(',      # ISNULL (T-SQL)
    ...
    mssql_score = 0
    for pattern in mssql_sygnaly:
        if re.search(pattern, sql_lower):
            mssql_score += 1

    # 3) Sygnały Postgresa (silniejsze)
    pg_sygnaly = [
        '::',                   # PostgreSQL cast
        'current_date',         # Postgres
        "interval '",           # INTERVAL
        '"public"',             # schema public
        '"rapdb"',              # schema rapdb
        'ilike',                # ILIKE (Postgres)
        ' limit ',              # LIMIT (na końcu zapytania PG; w MSSQL jest TOP na początku)
        'string_agg',           # (ma oba, ale częściej PG)
    ]
    pg_score = 0
    for pattern in pg_sygnaly:
        if pattern in sql_lower:
            pg_score += 1

    if mssql_score > pg_score:
        # Brak jawnej bazy — domyślnie ebayApiDB
...
def popraw_sql_po_bledzie(pierwotny_sql, blad, pytanie_pl, schema_text, schema_dict=None):
    ...
    realne_kolumny_info = ""
    if schema_dict:
        # Pattern: "kolumna X.Y nie istnieje" (gdzie X to alias, Y to kolumna)
        m = re.search(r'kolumna\s+(\w+)\.(\w+)\s+nie istnieje', blad, re.IGNORECASE)
        if m:
            alias = m.group(1)
            kolumna = m.group(2)
            # Znajdź jaka tabela ma ten alias w SQL
            # Pattern: "schema"."table" AS alias  lub  table AS alias
            ...
            if tabela_match:
                ...
                if kolumny:
                    # Znajdź podobne nazwy (mogą być literówki)
...
"""
        # Jeśli kilka błędów (np. dwa aliasy) — powtórz dla każdego
        ...
        if len(wszystkie_matche) > 1 and not realne_kolumny_info:
            # fallback: pokaż wszystkie tabele z SQL
...
"""
    ...
    prompt = f"""Jesteś ekspertem SQL dla PostgreSQL. Poprzednie zapytanie padło z błędem — popraw je MINIMALNIE.

SCHEMAT BAZY (skrócony, najważniejsze tabele):
{schema_text[:8000]}
...
KOMUNIKAT BŁĘDU Z POSTGRESQL:
{blad}
{realne_kolumny_info}

🔴 KRYTYCZNE INSTRUKCJE NAPRAWY:

1. **MINIMALNA ZMIANA**: Popraw TYLKO to co jest błędne. Zachowaj wszystkie pozostałe tabele, JOIN-y, WHERE, kolumny w SELECT. NIE przepisuj zapytania od zera. NIE zmieniaj strategii. NIE zmieniaj użytych tabel, jeśli błąd dotyczy tylko kolumny.

2. **WSKAZÓWKA HINT z PostgreSQL**: Jeśli błąd zawiera "HINT: Być może chodziło ci o X" — użyj DOKŁADNIE tego X.

3. **JEŚLI MASZ LISTĘ REALNYCH KOLUMN** (sekcja 🎯 powyżej) — WYBIERZ Z NIEJ WŁAŚCIWĄ. NIE WYMYŚLAJ własnych nazw.

4. **CZĘSTE PUŁAPKI NAZEWNICTWA w tej bazie**:
   - Identyfikatory kończące się na "ID" są PISANE WIELKIMI LITERAMI: "pakID", "artID", "zlecprID"
   - Identyfikatory złożone (camelCase) zachowują wielkie litery pośrodku: "proNrPartii", "pakUserCreateLogin"
   - ZAWSZE cytuj takie kolumny w cudzysłowach

5. **CUDZYSŁOWY OBOWIĄZKOWE**:
   - Nazwy z wielkimi literami MUSZĄ być w cudzysłowach: T1."proNrPartii"
   - Schematy i tabele też: "public"."wmsPakunkiHist"
   - Alias tabeli (T1, pak, poz) NIE w cudzysłowach — tylko kolumna po kropce

6. Zwróć TYLKO poprawione zapytanie SELECT, bez markdown, bez komentarzy, bez średnika.
"""
...
def odpal_zapytanie_sql(sql, dbname="maggo"):
    """Odpala zapytanie. Sama wykrywa czy to Postgres czy SQL Server."""
...
def odpal_z_self_healing(sql, pytanie_pl, schema_text, schema_dict=None, max_retries=3):
    ...
    for attempt in range(max_retries):
        status_box = st.empty()
        if attempt == 0:
            status_box.info(f"▶️ Próba {attempt + 1}/{max_retries}: odpalam zapytanie...")
        else:
            status_box.warning(f"🔄 Próba {attempt + 1}/{max_retries}: AI naprawia SQL po błędzie...")

        df, err = odpal_zapytanie_sql(current_sql)
        ...
        with st.spinner("AI analizuje błąd i poprawia SQL..."):
            poprawiony_sql, gen_err = popraw_sql_po_bledzie(current_sql, err, pytanie_pl, schema_text, schema_dict)
...
with tab_ai:
    ...
    with schema_col3:
        if st.button("🔵 Zbadaj MSSQL"):
            with st.spinner("Pobieram schemat SQL Server..."):
                try:
                    s = pobierz_schema_mssql()
                    st.session_state["_schema_mssql"] = s or {}
                    if s:
                        st.success(f"✅ SQL Server: {len(s)} obiektów")
                    else:
    ...
    with st.expander(f"📚 Schemat — 🟢 Postgres: {liczba_pg} | 🔵 MSSQL: {liczba_mssql}", expanded=False):
        tab_pg, tab_ms = st.tabs(["🟢 PostgreSQL (maggo)", "🔵 SQL Server (ebayApiDB, STEEPC, SHOP_PMG)"])
    ...
    if "ai_sql" in st.session_state:
        ...
        if c1.button("▶️ Odpal zapytanie", type="primary"):
            if not can_refresh:
                ...
                df_ai, err, attempts = odpal_z_self_healing(edited_sql, pytanie_orig, schema_text, schema_dict=schema, max_retries=3)
