...
with tab_bilanse:
    ...
    if "🚛" in bilans_typ:
        ...
        with col_b:
            koszty_pozostale_full = st.number_input("Koszty pozostałe / szt", value=800, step=50, key="koszty_poz_full",
                                                     help="Stała: robocizna + administracja + utrzymanie + transport")
        ...
        if dane is None:
            ...
            if df_sprz.empty or df_full.empty:
                ...
                df["sr_koszt_czesci"] = df["sr_koszt_czesci"].astype(float)
                df["koszty_pozostale"] = float(koszty_pozostale_full)
                df["marza_pln"] = df["sr_cena_pln"] - df["sr_koszt_czesci"] - df["koszty_pozostale"]
                df["marza_pct"] = (df["marza_pln"] / df["sr_cena_pln"] * 100).round(1)
                ...
                df_show = df.sort_values("marza_pln")[
                    ["skrzynia", "sztuk_sprzedanych", "sr_cena_w_walucie", "sr_cena_pln",
                     "sztuk_wyprodukowanych", "sr_koszt_czesci", "max_koszt_czesci", "koszty_pozostale",
                     "marza_pln", "marza_pct",
                     "prog_dzwonka", "dzwonil_razy"]
                ].copy()
                df_show.columns = [
                    "Skrzynia", "Sprzed. szt", "Śr. cena waluta", "Śr. cena PLN",
                    "Wyprod. szt", "Śr. koszt cz.", "Max koszt", "Koszty pozost.",
                    "Marża /szt", "Marża %",
                    "Próg dzwonka", "Dzwonił razy"
                ]
                ...
                **Co widzisz:**
                ...
                - **Śr. cena PLN** — przeliczona heurystyką 70/30 EUR/PLN (kurs {kurs_eur_full})
                - **Śr. koszt cz.** — średni koszt części pobranych z magazynu (system)
                - **Koszty pozost.** — stała wartość {koszty_pozostale_full} zł/szt (robocizna + admin + utrzymanie + transport)
                - **Marża /szt** = cena PLN - koszt części - koszty pozostałe
                - **Próg dzwonka** = max wartość poborów części przed alertem (per skrzynia)
                - **Dzwonił razy** = ile partii w 3mc przekroczyło próg
                
                **🚨 Czerwone wiersze** = strata (cena < koszt łączny), wymagają natychmiastowej interwencji.  
                **⚠️ Żółte** = niska marża <500 zł, drobny wzrost kosztu = strata.  
                **✅ Zielone** = OK marża.
                
                ⚠️ **Uwagi do interpretacji**:
                - "Czarna masa" Czarka (wydania z ręki używanych części) jest **poza systemem** — realny koszt może być +20-30%
                - Skupy z rynku (~1.6M zł / 10mc) NIE są jeszcze tu wkalkulowane (parsowanie wolnego tekstu skupów wymaga dopracowania)
                - System nie rozróżnia formalnie nowych od używanych części pobieranych z magazynu (Dariusz wskazał lokalizację MCU jako klucz — do zaimplementowania)
                """)
